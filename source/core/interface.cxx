...
itf_impl::itf_impl(mqtt_service &mqtt, itf_info &info)
    : info(info), topic_base("pza/" + info.group + "/" + info.device_name +
			     "/" + info.name),
      topic_cmd(topic_base + "/cmds/set"), mqtt(mqtt)
{
}
...
void itf_impl::register_attributes(
    const std::list<attribute::s_ptr> &attribute_list)
{
	std::mutex mtx;
