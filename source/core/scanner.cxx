...
scanner::scanner(mqtt_service &mqtt)
    : _mqtt(mqtt), _message_cb(nullptr), _condition_cb(nullptr),
      _pub_msg(nullptr)
{
}
...
int scanner::run()
{
	...
	if (_message_cb == nullptr || _condition_cb == nullptr ||
	    _pub_msg == nullptr || _sub_topic.empty()) {
	...
	ret = _cv.wait_for(lock, std::chrono::milliseconds(_scan_timeout),
			   _condition_cb);
