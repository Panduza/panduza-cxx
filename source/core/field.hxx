...
public:
    ...
    void set_value(const _type &value)
    {
        _value = value;
    ...
    void set_callback(const std::function<int(const nlohmann::json &data)> &callback)
    {
        _callback = callback;
    }
...
private:
    void _setJsonType()
    {
        ...
        else if (typeid(_type) == typeid(unsigned int))
        {
            _json_type = nlohmann::json::value_t::number_unsigned;
        }
