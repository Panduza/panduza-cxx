    ...
    : info(info)
{
	...
	scanner.set_scan_timeout_ms(interfaces_timeout)
	    ...
	    .set_publisher(mqtt::make_message("pza", info.bench + "/" + info.name))
	    .set_subscription_topic("pza/" + info.bench + "/" + info.name + "/+/atts/info");
    ...
    : _impl(std::make_unique<device_impl>(mqtt, info))
{
	...
	for (auto &itf : _impl->interfaces_scanned) {
		...
		auto itf_ptr = interface_factory::create_interface(mqtt, info.bench, info.name, itf.first, type);
		if (itf_ptr == nullptr) {
...
const std::string &device::get_bench() const
{
	return _impl->info.bench;
}
